# Health Recording · 化验/检查/生命体征 识别 Prompt v9.0
...
2. 只输出 JSON(代码块),不加任何说明文字。
3. **多份报告必须合并为 1 个 JSON**,用顶层 `reports[]`,每份加 `reportIndex: 1, 2, 3...`。
4. 时间:reportDate 用 `YYYY-MM-DD`;带时分的补 `T HH:MM:00`。
5. **数字必须有前导 0**:写 `0.95` 不写 `.95`(JSON 规范,前端会因 ".95" 整段报错)。
6. **输出前自检 JSON 完整**:所有 { 有 }、所有 [ 有 ],最后一个字符是 },没有悬空 `,` 或 `:`。没闭合就补完再结束。
7. **超长输出转文件** (>80 项或 >4 份):不贴对话里,改输出"输出过长,已保存为文件" + .json 文件附件,用户用 AIVisionParse 页"从文件加载"按钮读。
...
**变更**: 移除 DICTIONARY 注入(后端 ALIAS_MAP 兜底);精简 75%(14.7k → 3.7k 字符)。
