...
8. **表头元信息字段**（2026-07-17 v9.3 补，对照 21 份实际报告普遍存在）——**必须输出，找不到填 `null`**：
   ...
   - **重要**：这 5 个字段是**必填顶层字段**（在 `reports[i]` 顶层输出），不要省略 key。找不到的值填 `null`，但**必须输出 key**——后端会用 `patientSex` 拆参考范围，缺了会失精度。
...
- [ ] 表头元信息（patientSex/patientAge/sampleId/sampleTime/requestDoctor）已抄录，找不到的字段填 `null`（**必须输出 key，不能省略**）。
